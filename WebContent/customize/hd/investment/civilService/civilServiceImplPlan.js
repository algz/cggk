var civilServiceImplPlan = {

	getPanel : function(){
		var sm = new Ext.grid.CheckboxSelectionModel(); 
		var store = new Ext.data.Store({
			proxy : new Ext.data.HttpProxy({
						url : '../JSON/civilServiceImplPlanRemote.getGridData?d='+new Date(),
						method : 'post'
					}),
			...
			tbar : new Ext.Toolbar({
				items : ['-' ,{
							...
							store : comboStore
								...
								handler : function(){civilServiceImplPlan.appoint();}
								...
								handler : function(){civilServiceImplPlan.sendImplPlan();}
		...
		grid.on('afteredit',function(e){
			...
			if (rec.dirty) {
				...
				arr.push(rec.data);
				Ext.Ajax.request({
							url : '../JSON/civilServiceImplPlanRemote.editCivilServiceImplPlan?d='+new Date(),
							method : 'post',
							waitMsg : '数据加载中，请稍后....',
							params : {
	...
	appoint : function(){
		...
		var grid = Ext.getCmp('civilServiceImplPlanGrid');
		var arr = new Array();
...
//下发实施计划	
	sendImplPlan : function(){
		...
		Ext.MessageBox.confirm('提示','确定项目主管下发数据?',function(e){
			if(e=='yes'){
				Ext.Ajax.request({
					url : '../JSON/civilServiceImplPlanRemote.sendImplementPlan',
					method : 'post',
					waitMsg : '数据加载中，请稍后....',
					params : {
						implementplanid : Ext.util.JSON.encode(arr)
