			...
			handler : function() {
				...
				if (freeFlow == true) {
					...
					if (record == undefined) {
						...
						Untils.approvePanel.privilege(approvalParams, noPrivilege, success, failure);
					}
				}
			}
		}, {
 ...
 */
Untils.approvePanel.privilege = function(approvalParams, noPrivilege, success, failure) {
	if(!noPrivilege) {
		Ext.Ajax.request({
			url : '../JSON/approval_ApprovalRemote.filterDataId',
			method : 'POST',
			success : function(response, options) {
				...
				approvePanel.combPanel.hide();
				if(obj.success == 'all') {
					Untils.approvePanel.getTempletInfo(approvalParams, success, failure);
				} else if (obj.success == 'notall') {
					Ext.MessageBox.confirm('' + getResource('resourceParam575') + '',
						'部分数据没有权限进行审批：' + obj.message, function(btn, text){
						if(btn == 'yes'){
 ...
 * @param {} failure
 */
Untils.approvePanel.getTempletInfo=function(approvalParams, success, failure) {
	Ext.Ajax.request({
			...
			if (obj.success) {
				...
				if(nodetypeid == '6') {
					...
					var functOK = function(userid) {
						...
						Untils.approvePanel.selectTempletSubmit(approvalParams, success, failure);
					}
					// 创建审批节点确认界面，并展示
 ...
 * @param {} failure 提交失败的回调函数
 */
Untils.approvePanel.selectTempletSubmit = function(approvalParams, success, failure) {
	approvePanel.combPanel.show();
	Ext.Ajax.request({
		url : '../JSON/approval_ApprovalRemote.begin',
		method : 'POST',
...
Untils.approvePanel.submit = function(templetId, processName, approveNote, objectID, objectType, noPrevilige, success, failure) {
	if(!approvePanel.combPanel) {
		approvePanel.combPanel = new Ext.LoadMask(document.body, {
			msg : '' + getResource('resourceParam7131') + ''// 正在提交操作，请稍候...
		});
	...
	approvePanel.combPanel.show();
	Untils.approvePanel.privilege(approvalParams, noPrevilige, success, failure);
};
