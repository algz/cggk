	...
	getPanel : function(){
		...
		var cm = new Ext.grid.ColumnModel([sm, {
			...
			renderer : function(value, cellmeta, record, rowIndex) {
				return "<a href='javascript:void(0);' onclick=equipServiceContract.createContractWin('" + record.get('contractid') + "')>" + value
						+ "</a>";;
			}
		}, {
			header : '项目名称',
			...
			tbar : new Ext.Toolbar({
				items : ['-', {
					...
					store : comboStore
						...
						handler : function() {equipServiceContract.createContractWin("");}
					}, '-', {
						text : '修改',
						iconCls : 'edit1',
						disabled : main.CONTRACT,
						handler : function() {
							...
							equipServiceContract.createContractWin(recs[0].get('contractid'),true);
						}
					}, '-', {
						text : '删除',
						iconCls : 'del1',
						disabled : main.CONTRACT,
						handler : function() {equipServiceContract.deleteContract();}
					}, '-', {
						text : '送审',
						iconCls : 'icon-importTasks',
						disabled : main.CONTRACT,
						handler : function() {equipServiceContract.doApproval();}
					}]})
...
//			修改的列
	...
	createContractWin : function(id,flag){
		if("" == id){//新建
			equipServiceContract.isEdit = true;
		}else{//查看
			equipServiceContract.isEdit = false;
			if(flag){
				equipServiceContract.isEdit = true;
			}
		}
		var win = new Ext.Window({
			...
			items : [{
				...
				items : [{
							...
							items : [{
										...
										items : [{
													...
													xtype : 'hidden',
													name : 'contracttype',
													value : equipServiceContract.contracttype
												}, {
													xtype : 'hidden',
													id : 'equipregistId',
													name : 'equipregistId'
													...
													store : new Ext.data.Store({
														proxy : new Ext.data.HttpProxy({
																	url : '../JSON/equipServiceManageRemote.getProjectNums?d=' + new Date(),
																	method : 'POST'
																}),
														reader : new Ext.data.JsonReader({
																	totalProperty : 'totalProperty',
													...
													allowBlank : false,
													name : 'projectnum',
													projectDataType : equipServiceContract.contracttype,
													listeners : {
													...
													readOnly : !equipServiceContract.isEdit
												}, {
													xtype : 'combo',
													fieldLabel : '乙方',
													mode : "remote",
													...
													items : [{
																...
																listeners : {
																	focus : function(field) {
																		if (equipServiceContract.isEdit) {
				...
				fileUpload : true,
				items : [{
					xtype : !equipServiceContract.isEdit ? 'hidden' : 'fileuploadfield',
					id : 'form-file',
					fieldLabel : '上传附件',
					name : 'filename',
					anchor : '90%',
					...
					xtype : equipServiceContract.isEdit ? 'hidden' : 'displayfield',
					id : 'form-filename',
					name : 'filename',
					fieldLabel : '合同文件',
					listeners : {
						afterrender : function(field) {
							if("" != id){
								var grid = Ext.getCmp('equipServiceContractGrid');
			...
			buttons : [{
						text : '确定',
						hidden : !equipServiceContract.isEdit,
						handler : function() {
							var form = win.findById("contractEditorForm");
							var fileform = Ext.getCmp("fileFormPanel");
										...
										success : function(form, action) {
											...
											Ext.Ajax.request({
														...
														success : function(response, opts) {
															...
															if (obj.success == true) {
																win.close();
																Ext.getCmp('equipServiceContractGrid').store.load()
	...
	doApproval : function(){
		...
		if (stock_arr.length != 0) {
			approvePanel.submit('478106', "设备项目合同审批(股份)", "设备项目合同审批(股份)", stock_arr, 
				"DeviceContractmanagement", true, equipServiceContract.approvePanelSuccess,
				equipServiceContract.approvePanelFailure);
		}
		if (group_arr.length != 0) {
			approvePanel.submit('478150', "设备项目合同审批(集团)", "设备项目合同审批(集团)", group_arr, 
				"DeviceContractmanagementGroup", true, equipServiceContract.approvePanelSuccess,
				equipServiceContract.approvePanelFailure);
		}
	...
	approvePanelSuccess : function(){
		var rows = Ext.getCmp("equipServiceContractGrid").getSelectionModel().getSelections();
