...
contractManagement.mainGrid = Ext.extend(Ext.grid.EditorGridPanel, {// 定义主窗体,主窗体前缀带main
	...
	contracttype : 1,// 合同类型:1或空设备类型;2工程类型
			...
			win = new Ext.Window({
						...
						items : [{
									...
									loadForm : function(form, fileForm) {
										...
										form.getForm().findField('projectcategorys').setValue(rec.get('projectcategorys')==1?'新建':'大修');
...
//					}, 
						...
						renderer : function(value, cellmeta, record, rowIndex) {
							...
							if (status == 1) {
								...
								return '<span style="color:green">已退回</span>';
							} else {
								return value;
							}
						}
			...
			tbar : ['-', {
						...
						handler : function() {
							...
							if (stock_arr.length != 0) {
								utils.approvePanel.submit('478106', "设备项目合同审批(股份)", "设备项目合同审批(股份)", stock_arr, "DeviceContractmanagement", true, function() {
							...
							if (group_arr.length != 0) {
								utils.approvePanel.submit('478150', "设备项目合同审批(集团)", "设备项目合同审批(集团)", group_arr, "DeviceContractmanagementGroup", true, function() {
											// Ext.Msg.alert('提示', '送审成功!');
											grid.store.load();
										}, function() {
											// Ext.Msg.alert('提示', '提交失败!');
...
contractManagement.contractEditorForm = Ext.extend(Ext.Panel, {
			...
			contracttype : 1,// 合同类型:1设备类型;2工程类型
