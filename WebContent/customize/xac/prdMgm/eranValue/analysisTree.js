...
analysisTree.init=function(transferFlag)
{
	...
	analysisTree.treePanel = new Ext.tree.TreePanel( {
		id : 'analysisTree',
		width : 398,
		lines : true,
		split : true,
...
//					Ext.EventObject.ctrlKey,false);// 点击该node
	...
	if (transferFlag == "coop") {
		analysisTree.treePanel.on("beforeappend", beforeAppend);
		analysisTree.treePanel.on("nodedragover", function(e) {
		});
		analysisTree.treePanel.on("beforenodedrop", function(e) {
					...
					success : function(response, options) {
						...
						if (obj.success == true) {
							...
							if (parentNode.contains(targetNode)) {
								analysisTree.nodeId = parentNode.id;
							} else if (targetNode.contains(parentNode)) {
								analysisTree.nodeId = targetNode.id;
							} else {
								analysisTree.nodeId = 0;
							}
							collarbMain.refresh();
							// var dropNode = e.dropNode;
					// var targetNode = e.target;
