...
Ext.ux.ItemSelector = Ext.extend(Ext.form.Field, {
	...
	onRender : function(ct, position) {
		...
		if (!this.readOnly) {
			if (!this.toSortField) {
				this.toTopIcon.on('click', this.allTo, this);
				this.upIcon.on('click', this.up, this);
				this.downIcon.on('click', this.down, this);
				this.toBottomIcon.on('click', this.allBack, this);
			}
		...
		//
		this.hiddenFieldName = this.el.createChild(hiddenTagName);
	...
	allTo : function() {
		...
		for (var i = 0; i < fromStore.getCount(); i++) {
			...
			if (undefined != record.get("userid")) {
				for (var j = 0; j < this.toMultiselect.view.store.getCount(); j++) {
					rec = this.toMultiselect.view.store.getAt(j);
					if (record.get("userid") == rec.get("userid")) {
						flag = false;
	...
	allBack : function() {
		...
		for (var i = 0; i < toStore.getCount(); i++) {
			...
			if (undefined != record.get("userid")) {
				for (var j = 0; j < this.fromMultiselect.view.store.getCount(); j++) {
					rec = this.fromMultiselect.view.store.getAt(j);
					if (record.get("userid") == rec.get("userid")) {
						flag = false;
						this.toMultiselect.view.store.remove(record);
						continue;
	...
	fromTo : function() {
		...
		if (selectionsArray.length > 0) {
			for (var i = 0; i < selectionsArray.length; i++) {
				...
				var flag = true;
				if (undefined != record.get("userid")) {
					for (var x = 0; x < this.toMultiselect.view.store
							.getCount(); x++) {
						rec = this.toMultiselect.view.store.getAt(x);
	...
	toFrom : function() {
		...
		if (selectionsArray.length > 0) {
			for (var i = 0; i < selectionsArray.length; i++) {
				...
				var flag = true;
				if (undefined != record.get("userid")) {
					for (var x = 0; x < this.fromMultiselect.view.store
							.getCount(); x++) {
						rec = this.fromMultiselect.view.store.getAt(x);
						if (record.get("userid") == rec.get("userid")) {
							flag = false;
							this.toMultiselect.view.store.remove(record);
						}
	...
	valueChanged : function(store) {
		...
		// if (store.getAt(i).get('userid') == store.getAt(j)
		// .get('userid')) {
		...
		for (var i = 0; i < store.getCount(); i++) {
			...
			// valuesName.push(record.get(this.displayField));
		}
	...
	onRowDblClick : function(vw, index, node, e) {
		...
		// // alert(outer.innerHTML);
		// alert(vw + index + node.innerHTML + e);
		// // alert(index);
		// alert(this.fromMultiselect.view.store.getAt(1).get('truename'));
	...
	reset : function() {
		...
		if (!this.allowDup) {
			this.fromMultiselect.store.reload();
