var tableDefineTreeGrid = {}
tableDefineTreeGrid.init = function() {
	var self = this;
	...
	var grid = new Ext.ux.maximgb.tg.sysEditTreeGridPanel({
		...
		listeners : {
			...
			beforeedit : function(e) {
				...
				if (e.record.get("primaryKey") == true) {
					if (e.record.get("pkGenType") == 2
							|| e.record.get("pkGenType") == 3) {

						if (dataIndex == "length" || dataIndex == "precision"
								|| dataIndex == "scale"
								|| dataIndex == "datatypename"
								|| dataIndex == "nullable"
								|| dataIndex == "defaultValue") {
