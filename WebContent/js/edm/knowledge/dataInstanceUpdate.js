						...
						callSeam("datacenter_DataCenterRemote",
								...
								function(result) {
									...
									if (1 == flag) {
										...
										// description
										// .getValue()
										var node = cateKowleInstancePanel.attributeTree
												.getNodeById(ids);
//										var p1n = node.parentNode;
//										var p2n = p1n.parentNode;
//
//										var root = cateKowleInstancePanel.attributeTree.root;

//										cateKowleInstancePanel.attributeTree.updatenodetext(
//												vo.getCategoryInstanceID(),
//												vo.getCategoryInstanceName(),
//												vo.getDescription());
										node.setText(vo.getCategoryInstanceName());
										node.attributes.description=vo.getDescription();
										/*
...
//									button.enable();
									...
									/**
									 * 更新树形结构从当前节点到顶级节点的版次标识 var nodeid =
									 * window.parent.getCheckNode().id;
									 * window.parent.reload(function() {
									 * cateInstanceTree.root.reload( function() {
									 * var node =
									 * window.parent.getNodeById(nodeid); var
									 * newText = node.attributes.realName +
									 * node.text.substring(node.text.indexOf("\/"));
									 * 
									 * cateInstanceTree.root.setText(newText);
