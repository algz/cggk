...
objectWin.init = function() {
	var addobj = new Ext.Button({
		id : 'addobj',
			...
			'click' : function(button) {
				...
				for (var i = 0; i < count; i++) {
					...
					if ("" == record.get("icon")) {
						...
						iconpath = '../base/icons/edm/' + record.get("icon");
					}
				...
				button.enable();
				objectWin.checkWin.close();
			}
		}
	});
	var strurl = '../JSON/dataClassification_DataClassificationRemote.getDataClassificationList?name=&type=1';
	...
	var tb = null;
	objectWin.grid = objectWin.gridinit(ds, cm, tb, sm);
	myGrid.loadvalue(objectWin.grid.store, objectWin.args, objectWin.baseargs);
	var min = new Ext.form.NumberField({
				fieldLabel : '' + getResource('resourceParam1767') + '',
				anchor : '95%',
				labelAlign : 'right',
				labelWidth : 20,
				width : 200,
	...
	var max = new Ext.form.TextField({
				fieldLabel : '' + getResource('resourceParam1768') + '',
				anchor : '95%',
				labelAlign : 'top',
				labelWidth : 20,
	...
	var label = new Ext.form.Label({
				text : '' + getResource('resourceParam1746') + ':'// 对象名称
			})
	...
	objectWin.checkWin = new Ext.Window({
				width : 450,
				height : 390,
				tbar : ['->', label, name, query],
				title : '' + getResource('resourceParam1771') + '',
				modal : true,
				// layout : ',
				items : [objectWin.grid, panel],
				buttons : [addobj,{
	...
	objectWin.checkWin.show();
	objectWin.checkWin.on("close", function() {
				objectWin.checkWin.destroy();
			});

}

objectWin.gridinit = function(ds, cm, tb, sm, plugin) {
