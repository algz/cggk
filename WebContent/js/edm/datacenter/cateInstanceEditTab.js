...
cateInstanceEditTab.init = function(title, id, i, datacenterid, revision, isref) {
	...
	var tab1 = new Ext.Panel({
					...
					if (1 == isref) {
						...
						callSeam("privilege_DataPrivilegeRemote",
								...
								[opertationVo], function(result) {
									...
									if (obj.del == false) {
										Ext.getCmp('dataObjectColumnTreeDel'
												+ i).disable();
									} else {
										Ext.getCmp('dataObjectColumnTreeDel'
												+ i).enable();
									...
									if (obj.modifydata == false) {
										dataObjectTree.on('beforeedit',
												disableEdit);

										Ext.getCmp('dataObjectColumnTreeAdd'
												+ i).disable();
										Ext.getCmp('dataObjectColumnTreeSave'
												+ i).disable();
										Ext
												.getCmp('dataObjectColumnTreeRefresh'
														+ i).enable();
										Ext.getCmp('history_button' + i)
												.enable();
									} else {
										dataObjectTree.un('beforeedit',
												disableEdit);
		...
		tbar : [{
			...
			handler : function() {
				...
				// var url =
				// "../JSON/datacenter_DataCenterRemote.getDataCategoryMetaById?categoryid="
				...
				// return;
				/**
				 * var Plant = dataObjectTree.getStore().recordType; var p =
				 * null; p = new Plant({ text : '' +
				 * getResource('resourceParam1075') + '', dataEntityTypeName : '' +
				 * getResource('resourceParam1079') + '', dataEntityType :
				 * 'string', inout : '0', leaf : true, parent : null, isRef :
				 * '0', value : '', dimension : '1', isArray : false });
				 * dataObjectTree.stopEditing(); p.markDirty() // var n1 =
...
//				}
			...
			handler : function(button) {
				...
				for (var i = 0; i < store.getModifiedRecords().length; i++) {
					...
					var name = modifiedRecord.get('text');
					name = name.replace(/(^\s*)|(\s*$)/g, '');

					if (modifiedRecord.get("dataEntityID") != undefined) {
						dataEntity.setDataEntityType(modifiedRecord
								.get('dataEntityType'));
						// dataEntity.setDataEntityName(modifiedRecord.get('text'));
						dataEntity.setDataEntityName(name);
						...
						dataEntity
								.setDimension(modifiedRecord.get('dimension'));
						dataEntity.setValue(modifiedRecord.get('value'));
						dataEntity.setDataEntityID(modifiedRecord
								.get('dataEntityID'));
						dataEntity.setParentDataEntityID(modifiedRecord
								.get('parentDataEntityID'));
						dataEntity.setCustomTypeItemID(modifiedRecord
								.get('customTypeItemID'));
						dataEntity.setCustomTypeParentID(modifiedRecord
								.get('customTypeParentID'));
						dataEntity.setInout(modifiedRecord.get('inout'));
						dataEntity.setFileID(modifiedRecord.get('fileID'));
						dataEntity.setDescription(modifiedRecord
								.get('description'));
				...
				for (var j = 0; j < addlist.length; j++) {
					var name = addlist[j].get('text');
					name = name.replace(/(^\s*)|(\s*$)/g, '');
					var dataEntity = Seam.Remoting
							.createType("com.sysware.edm.dataentity.DataEntity");
					dataEntity.setDataEntityType(addlist[j]
							.get('dataEntityType'));
					dataEntity.setDataEntityName(name);
					...
					dataEntity.setDataCenterID(temp.datacenterid);
					dataEntity
							.setDimension(addlist[j].get('dimension') == undefined
									? "1"
				...
				for (var k = 0; k < removedRecords.length; k++) {
					if (removedRecords[k].get('dataEntityID') != undefined
						...
						var dataEntity = Seam.Remoting
								.createType("com.sysware.edm.dataentity.DataEntity");
						dataEntity.setDataEntityID(removedRecords[k]
								.get('dataEntityID'));
						dataEntity.setDataCenterID(temp.datacenterid);
							...
							if (result != 'false') {
								...
								window.parent.reload(function() {
									...
									if (null == cateInstanceTree.checkinstancenode) {
										cateInstanceTree.checkinstancenode = cateInstanceTree.attributeTree
												.getRootNode();
									}
									cateInstanceTree
											.updateParent(cateInstanceTree.checkinstancenode);
									...
									window.parent.document
											.getElementById("center_frame").firstChild.firstChild.innerHTML = node.attributes.realName
			 ...
			 * '-', { id : 'dataTagApproveMenu' + i, text : '' +
			 * getResource('resourceParam1375') +
			 * getResource('resourceParam1062') + '', menu : [new Ext.Action({
			 * id : 'dataTagApprovePass' + i, text : '' +
			 * getResource('resourceParam1365') + '', handler : function() {
			 * Ext.Msg.confirm('' + getResource('resourceParam575') + '',
			 * getResource('resourceParam9108') , function(btn) { if (btn ==
			 * 'yes') { Ext.Ajax.request({ url :
			 * "../JSON/datacenter_DataCenterRemote.approve", method : 'POST',
			 * success : function(response, options) { var obj = Ext.util.JSON
			 * .decode(response.responseText); }, disableCaching : true,
			 * autoAbort : true, params : {} }); } }); } }), new Ext.Action({ id :
			 * 'dataTagApproveSubmit' + i, text : '' +
			 * getResource('resourceParam1550') + '...', handler : function() {
			 * dataObjectTree.hide(); tab1.setAutoScroll(true);
			 * tab1.getTopToolbar().disable();
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel').on(
			 * 'beforetabchange', disableEdit); var callback = function() {
			 * tab1.setAutoScroll(false);
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel').un(
			 * 'beforetabchange', disableEdit); Ext.getCmp('approvePanel' +
			 * temp.id).hide(); dataObjectTree.show();
			 * tab1.getTopToolbar().enable(); }; if (Ext.getCmp('approvePanel' +
			 * temp.id)) { Ext.getCmp('approvePanel' + temp.id).show(); } else {
			 * approvePanel.init(tab1, temp.id, 'DataTagDataType', callback, '' +
			 * getResource('resourceParam1989') + '', callback); }
			 * tab1.doLayout(); } }), new Ext.Action({ id :
			 * 'dataTagApproveComment' + i, text : '' +
			 * getResource('resourceParam1448') + '', handler : function() {
			 * dataObjectTree.hide(); tab1.setAutoScroll(true);
			 * tab1.getTopToolbar().disable();
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel').on(
			 * 'beforetabchange', disableEdit); if (Ext.getCmp('commentGrid' +
			 * temp.id)) { Ext.getCmp('commentGrid' + temp.id).show(); } else {
			 * examApproval.getCommentGrid(tab1, temp.id, 'DataTagDataType',
			 * function() { tab1.setAutoScroll(false);
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel')
			 * .un('beforetabchange', disableEdit); Ext.getCmp('commentGrid' +
			 * temp.id).hide(); dataObjectTree.show();
			 * tab1.getTopToolbar().enable(); }); } tab1.doLayout(); } })] },
			 * '-', { id : 'dataEntityApproveMenu' + i, text : '' +
			 * getResource('resourceParam474') +
			 * getResource('resourceParam1062') + '', menu : [new Ext.Action({
			 * id : 'dataEntityApprovePass' + i, text : '' +
			 * getResource('resourceParam1365') + '', handler : function() { var
			 * sm = dataObjectTree.getSelectionModel(); var selections =
			 * sm.getSelections(); var ucount = 0; for (var i = 0; i <
			 * selections.length; i++) { var pNode = dataObjectTree.getStore()
			 * .getNodeParent(selections[i]); if (!pNode) { ucount = ucount + 1; } }
			 * if (ucount > 1) { Ext.MessageBox.show({ title : '' +
			 * getResource('resourceParam596') + '!', msg : '' +
			 * getResource('resourceParam7003') + ''//一次只能 +
			 * getResource('resourceParam503') + '' +
			 * getResource('resourceParam7004') + ''//一 +
			 * getResource('resourceParam455') + '' +
			 * getResource('resourceParam474') + '！', buttons :
			 * Ext.MessageBox.OK, icon : Ext.MessageBox.ERROR }); return; } if
			 * (sm.getCount() == 0) { Ext.MessageBox.show({ title : '' +
			 * getResource('resourceParam596') + '!', msg : '' +
			 * getResource('resourceParam459') + '一' +
			 * getResource('resourceParam455') + '' +
			 * getResource('resourceParam474') + '！', buttons :
			 * Ext.MessageBox.OK, icon : Ext.MessageBox.ERROR }); return; }
			 * Ext.Msg.confirm('' + getResource('resourceParam575') + '',
			 * getResource('resourceParam9108'), function(btn) { if (btn ==
			 * 'yes') { Ext.Ajax.request({ url :
			 * "../JSON/datacenter_DataCenterRemote.approve", method : 'POST',
			 ...
			 * .decode(response.responseText); }, disableCaching : true,
			 * autoAbort : true, params : {} }); } }); } }), new Ext.Action({ id :
			 * 'dataEntityApproveSubmit' + i, text : '' +
			 * getResource('resourceParam1550') + '...', handler : function() {
			 * var sm = dataObjectTree.getSelectionModel(); var selections =
			 * sm.getSelections(); var ucount = 0; for (var i = 0; i <
			 * selections.length; i++) { var pNode = dataObjectTree.getStore()
			 * .getNodeParent(selections[i]); if (!pNode) { ucount = ucount + 1; } }
			 * if (ucount > 1) { Ext.MessageBox.show({ title : '' +
			 * getResource('resourceParam596') + '!', msg : '一次只能' +
			 * getResource('resourceParam503') + '一' +
			 * getResource('resourceParam455') + '' +
			 * getResource('resourceParam474') + '！', buttons :
			 * Ext.MessageBox.OK, icon : Ext.MessageBox.ERROR }); return; } if
			 * (sm.getCount() == 0) { Ext.MessageBox.show({ title : '' +
			 ...
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel').un(
			 * 'beforetabchange', disableEdit); Ext.getCmp('approvePanel' +
			 * sm.getSelected().get('dataEntityID')) .hide();
			 * dataObjectTree.show(); tab1.getTopToolbar().enable(); }; if
			 * (Ext.getCmp('approvePanel' +
			 * sm.getSelected().get('dataEntityID'))) {
			 * Ext.getCmp('approvePanel' + sm.getSelected().get('dataEntityID'))
			 * .show(); } else { approvePanel.init(tab1, sm.getSelected()
			 * .get('dataEntityID'), 'DataEntityDataType', callback, '' +
			 * getResource('resourceParam1988') + '', callback); }
			 * tab1.doLayout(); } }), new Ext.Action({ id :
			 * 'dataEntityApproveComment' + i, text : '' +
			 * getResource('resourceParam1448') + '', handler : function() { var
			 * sm = dataObjectTree.getSelectionModel(); var selections =
			 * sm.getSelections(); var ucount = 0; for (var i = 0; i <
			 * selections.length; i++) { var pNode = dataObjectTree.getStore()
			 * .getNodeParent(selections[i]); if (!pNode) { ucount = ucount + 1; } }
			 ...
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel').on(
			 * 'beforetabchange', disableEdit); if (Ext.getCmp('commentGrid' +
			 * sm.getSelected().get('dataEntityID'))) { Ext.getCmp('commentGrid' +
			 * sm.getSelected().get('dataEntityID')).show(); } else {
			 * examApproval.getCommentGrid(tab1, sm.getSelected()
			 * .get('dataEntityID'), 'DataEntityDataType', function() {
			 * tab1.setAutoScroll(false);
			 * Ext.getCmp('cateInstanceAttriTab_tabPanel')
			 * .un('beforetabchange', disableEdit); Ext.getCmp('commentGrid' +
			 * sm.getSelected() .get('dataEntityID')) .hide();
			 * dataObjectTree.show(); tab1.getTopToolbar().enable(); }); }
			 * tab1.doLayout(); } })] },
					...
					bbar : ['->', {
						...
						handler : function() {
							...
							self.setHandler(function() {
								...
								var nodeid = cateInstanceAttriTab.params.nodeid
								...
								Seam.Component
										...
										.getAllChildCateByFath(nodeid,
												function(result) {
													var p = cateInstanceAttriTab.params.parentContainer
															|| cateInstancePanel.centerPanel;
													var tba = p.items.get(0);
													if (tba) {
														p.remove(tba);
													}
													var newPanel = cateInstanceAttriTab
															.init(nodeid);
													p.add(newPanel);
													p.doLayout();
												});
