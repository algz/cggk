...
historyDataObjectView.init = function(dataEntityId, revision) {
	var historyDataObjectView = {};
	var t1 = dataObjectTreeGrid.init();
	t1.getBottomToolbar().removeAll();
	t1.getColumnModel().setRenderer(4, function(v, p, r) {
...
historyDataObjectView.initCategoryHistoryButton = function(t,
	...
	var b = new Ext.Button({
		...
		handler : function() {
			if (b.getText().indexOf('' + getResource('resourceParam1240') + '') > 0) {// 点击
					...
					bbar : ['->', {
						...
						handler : function() {
							...
							b.setText(getResource('resourceParam1706') + ':'
									+ r.get('version') + ' '
									+ getResource('resourceParam1240') + ''// 点击
...
historyDataObjectView.initDataHistoryButton = function(t) {
	var b = new Ext.Button({
				text : '' + getResource('resourceParam576') + ''
						+ getResource('resourceParam1943') + ''// 历史
