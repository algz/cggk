...
tMain.toTreeView = function toTreeView(id, name) {
...
//	Ext.getCmp('view1_create_6').setVisible(true);
	...
	if (tMain.templatePanel) {
		tMain.mainPanel.remove(tMain.templatePanel);
	}
	tMain.templatePanel = new Ext.Panel({
				layout : 'fit',
				border : false,
				frame : false,
				autoScroll : true,
	...
	tMain.mainPanel.insert(6, tMain.templatePanel);
	...
	tMain.mainPanel.getLayout().setActiveItem(6);
}
...
//		Ext.getCmp('view1_create_6').setVisible(false);
		wbsPanel.refresh();
		tMain.mainPanel.getLayout().setActiveItem(0);
	}
	...
	var createContent = new Ext.Action({
				...
				handler : function() {
					...
					/***end***/
					tMain.mainPanel.getLayout().setActiveItem(2);
				}
			});
	var createTemplate = new Ext.Action({
				...
				handler : function() {
					...
					tMain.mainPanel.getLayout().setActiveItem(3);
				}
			});
	...
	var action1 = new Ext.Action({
				...
				handler : function() {
					/**
					 * bug编号804 wangyf
					 * bug信息：3如果选择一条模板信息后，再点新建建按钮，页面上的修改，删除，权限与返回按钮都应置恢。
					 *		4说明修改与权限页面也存在此问题
					 * 2011-05-24
					 */
					Ext.getCmp('view1_delete').disable();
					...
					approvePanel.reset();
					tMain.mainPanel.getLayout().setActiveItem(4)
					/**
					 * bug编号652 —— 送审（送审模板）
...
//					createSubLevelTask, createApprovalTask, createFromTemplate]
			...
			handler : function() {
				...
				/**
				 * bug编号590 wangyf
				 * bug信息：在系统管理——》任务流程模板管理中审批中的模板不能进行修改,其他状态可以进行修改
				 * 2011-05-17
				 */
				/****begin****/
				...
				if (wbsPanel.dataType == 'WBSContent') {
					 ...
					 */
					/***begin***/
					templateContent.contentForm.setTitle(getResource('resourceParam9799'), "");
					/***end***/
					tMain.mainPanel.getLayout().setActiveItem(2);
...
//						});
				...
				'-', {
					...
					handler : function() {
						...
						tMain.mainPanel.getLayout().setActiveItem(5);
...
//						});
	...
	wbsPanel.refresh();
	tMain.detailPanel = new Ext.Panel({});

	tMain.approvalPanel = approvePanel.init(null, "", "TemplateDataType",
			backToMain, "" + getResource('resourceParam943') + ""
					+ getResource('resourceParam1826') + "", backToMain);
	setDataPrivilege.privileged = tMain.privilege;
	tMain.privilegeSet = setDataPrivilege.init();
...
//				autoScroll : true,
				...
				items : [tMain.wbsPanel, tMain.detailPanel,
						templateContent.init(templateContentConfig),
						template.init(templateConfig), tMain.approvalPanel,// 审批面板
						tMain.privilegeSet, tMain.templatePanel]
			});
...
Ext.onReady(tMain.init, tMain, true);
