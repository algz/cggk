...
processGrid.getGrid = function() {
	...
	processGrid.grid = myGrid.initByPageSize(ds, cm, null, sm, processWest.limit, false);

	processGrid.grid.width = 5000;
	// processGrid.grid.width = processMain.panel.getInnerWidth();
	// grid.autoWidth = true;
	//processGrid.grid.height = 484;
	processGrid.grid.height = 484;
	processGrid.grid.enableColumnMove = false;
//	processGrid.grid.on('rowclick', function(thisgrid) {
...
//			var tag2 = record.data.startmonth2/6;
			...
			if (completeratiov && completeratiov!='') {
				p.body = '<div style="min-height:35px;height:35px;position:relative;top:-18px;margin: 0px;padding:0px;"><table width:100% border=0 cellspacing=3 cellpanding=1>'
						+ '<tr>'
						+ '<td width=250px>'
