...
<f:view >
  	<h:form enctype="multipart/form-data" id="fm" onsubmit="javaScript:return check();">
	    <%=session.getAttribute("resourceParam_446")%>：<s:fileUpload id="data" 
                                 data="#{common_FileUpload.data}" 
                                 contentType="#{common_FileUpload.contentType}" 
		     accept="images/png,images/jpg,application/ms-word,application/ms-excel"  
                               fileName="#{common_FileUpload.fileName}"

							fileSize="#{common_FileUpload.fileSize}"/>
	<h:inputHidden id='cid' value="#{common_FileUpload.cid}"/>					
	 <h:commandButton value="<%=session.getAttribute("resourceParam_447")%>" action="#{common_FileUpload.doUpload}" id="uploadFile"/>
           </h:form>
</f:view>
