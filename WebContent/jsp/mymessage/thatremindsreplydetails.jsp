...
<script src="../base/basejsp.js" type="text/javascript"></script>

</head>

<body oncontextmenu="self.event.returnValue=false" >
<f:view>
<div id="log_detail" class="log_detail">
<div id="goback" class="goback"><a href="thatremindsreply.seam?status=<h:outputText value="#{messageReceiveVo.allstatus}" />"><%=session.getAttribute("resourceParam_450")%></a></div>
		...
		<rich:column>
			<div id="log_box<h:outputText value="#{news.level}" />" class="log_box">
                <div id="log_title<h:outputText value="#{news.level}" />" class="log_title">
                   <a href="javascript:void(0);"><h:outputText value="#{news.truename}" /></a> 提出一<%=session.getAttribute("resourceParam_539")%>对<h:outputText value="#{news.itemTypeName}" />[<font color="red"><h:outputText value="#{news.taskname}" /></font>]<%=session.getAttribute("resourceParam_1884")%>
                   <a href="javascript:void(0)"><h:outputText value="#{news.messagetitle}" /></a>&nbsp; 
...
<!--                </div>-->
			    <div id="log_link2<h:outputText value="#{news.level}" />" class="log_link2">
                    <a href="javascript:void(0);"	onclick="showReply(<h:outputText value="#{news.level}" />);">
            ...
            <!-- 2级文章<%=session.getAttribute("resourceParam_1869")%>区域 start-->
			   <div class="reply_area" id="reply<h:outputText value="#{news.level}"/>" style="display:block;">
	         ...
	         <form id="feedbackform<h:outputText value="#{leafnews.level}" />" name="feedbackform<h:outputText value="#{leafnews.level}" />" action="../../MessageReceiveServlet" method="post" enctype="multipart/form-data"> 
                    <div id="log_reply_content<h:outputText value="#{leafnews.level}"/>" class="log_reply_content">
                       ...
                       <textarea id="content<h:outputText value="#{leafnews.level}"/>" name="content<h:outputText value="#{leafnews.level}"/>" 
                       		style="width: 400px; height: 100px;" onkeyup="warnThatReplyFour(this.value, <h:outputText value="#{leafnews.level}"/>)"></textarea>
                   		<div id="warnThatReplyFour<h:outputText value="#{leafnews.level}"/>"></div>
                    </div>
        ...
        <form id="feedbackform<h:outputText value="#{news.level}" />" name="feedbackform<h:outputText value="#{news.level}" />" action="../../MessageReceiveServlet" method="post" enctype="multipart/form-data"> 
...
<!--            <div class="log_content"></div>-->
			<div id="log_reply_all_content<h:outputText value="#{news.level}" />">
				<!-- bug编号471 wangyf 2011-05-12 -->
			   <textarea id="allcontent<h:outputText value="#{news.level}" />" name="allcontent<h:outputText value="#{news.level}" />" 
			   		style="width: 400px; height: 100px; margin:20px 0px 0px 0px;" onkeyup="warnThatFour(this.value)"></textarea>
            </div>
            <div id="warnThatFour"></div>
			<div id="log_reply_all_submit<h:outputText value="#{news.level}" />">
