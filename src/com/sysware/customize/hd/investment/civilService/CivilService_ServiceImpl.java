...
@Name("civilService_CivilService_ServiceImpl")
@SuppressWarnings({"unused","unchecked","deprecation"})
public class CivilService_ServiceImpl implements CivilService_Service{
	
	@In(create = true, value = "civilService_CivilService_DaoImpl")
	private CivilService_Dao civilServiceDao;
	
	
	
	//获取物料种类列表
	public List<CivilServiceVo> getWlzl(CivilServiceVo model, Pager pager) {
		//List<Object[]> wlzlList = civilServiceDao.getThisModel(model,pager);
		List<Object[]> wlzlList = null;
		List<CivilServiceVo> resultList = new ArrayList<CivilServiceVo>();
		List<CivilServiceVo> finalResultList = new ArrayList<CivilServiceVo>();
		CivilServiceVo vo=null;
...
*/			
		...
		{
			CivilServiceVo vo1 = resultList.get(0);
			finalResultList.add(vo1);
			
			for(int i=1;i<resultList.size();i++)
			{
				CivilServiceVo vo2 = new CivilServiceVo();
				boolean flag = false;
				...
				{
					CivilServiceVo vo3 = new CivilServiceVo();
	...
	public int getWlzlCount(CivilServiceVo vo) {
		return civilServiceDao.getCount(vo);
	}
	
	//添加
	public String addThisModel(CivilServiceModel model) throws Exception {
		return civilServiceDao.addThisModel(model);
	}
	...
	public void delWlzl(String id)  throws Exception {
		civilServiceDao.delThisModel(id);
	...
	public String updateThisModel(CivilServiceModel model)  throws Exception {
		return civilServiceDao.updateThisModel(model);
	}
	
	/**
	 * 查询物料种类
	 */
	public List<CivilServiceVo> searchWlzl(String materialCode,String name,String designation,String standard,String techniccondition,Pager pager,CivilServiceVo vo) {
		List<CivilServiceVo> list = civilServiceDao.searchThisModel(materialCode,name,designation,standard,techniccondition, pager, vo);
		return list;
	}
	
	//获取物料名称查询记录条数
	public int getSeachWlzlCount(String name){
		int count = civilServiceDao.getSeachCount(name);
		return count;
	}
	...
	public int getCountByNodeId(String nodeid) {
		int count = civilServiceDao.getCountByNodeId(nodeid);
		return count;
	}
	...
	public CivilServiceModel excelContentsSaveDb(CivilServiceModel model) throws Exception {
		/*int flag = civilServiceDao.excelContentsSaveDb(tm);
		return flag;*/
		return civilServiceDao.excelContentsSaveDb(model);
	}
	...
	public Object[] fileNameSaveDb(CivilServiceModel model) throws Exception {
		Object[] returnArrayObject = civilServiceDao.fileNameSaveDb(model);
	...
	public String findIdByName(String ename) {
		return civilServiceDao.findIdByName(ename);
	}
	...
	public Object[] sheetNameSaveDb(CivilServiceModel model) throws Exception {
		return civilServiceDao.sheetNameSaveDb(model);
	}
	...
	public void ImportExcelFile(List<FileItem> items, String nodeId, String nodeIdForGYS) throws NoIsMicroSoftExcelException,AppException,BusinessException{
		...
		CivilServiceModel tm = null;
		Workbook workbook = null;
		Workbook workbook2 = null;
		CivilServiceModel returnVo = null;//保存后,clone后,返回的vo
		CivilServiceVo tbequipmenttypevo = null;
		...
		while(itr.hasNext()){
			...
			//1、文件名入库操作
			try {
				arrayObj = civilServiceDao.fileNameSaveDb(tm);//1.Excel入库
			} catch (Exception e) {
				e.printStackTrace();
			...
			newTm = (CivilServiceModel)arrayObj[0];
			//String id = civilServiceDao.findIdByName(prename,newTm);
			...
			CivilServiceModel newTm2 = new CivilServiceModel();
			...
			for (String str : sheetNameArr) {
				...
				tm.setIsLeaf(0);
				try {
					arrayObj2 = civilServiceDao.sheetNameSaveDb(tm);//2.Sheet入库
			...
			try {
				...
				for (int i = 0; i < sheetarr.length; i++) {
					...
					//String sheetid = civilServiceDao.findIdByName(sheetname);
					...
					for (int m = 2; m < rows; m++)//遍历行
					{
						...
						try {
							...
							if (1>2) {
								...
								returnVo = civilServiceDao.excelContentsSaveDb(tm);
								...
								/*if(returnVo != null){//returnVo为null表示不需要处理
									...
									if(tbequipmenttypevo != null){
										civilServiceDao.dealTheManufacture(tbequipmenttypevo);
									}
									//2011-10-13 插入  器材明细账表(TB_EQUIPMENTACCOUNT)
									if(tbequipmenttypevo != null){
										civilServiceDao.insertEquipmentAccount(tbequipmenttypevo);
									}
								}*/
							}
	...
	public String getNameByNodeId(String nodeid) {
		String name = civilServiceDao.getNameByNodeId(nodeid);
		return name;
	}
	

	public int getSubChildCount(String id) {
		int count = civilServiceDao.getSubChildCount(id);
		return count;
	}

	public void deleteAllWlzlByNode(String id) throws AppException,Exception {
		civilServiceDao.deleteAllByNode(id);
	}
	...
	public void updateNodestatus() throws Exception{
		civilServiceDao.updateNodestatus();
	}
	...
	public void updateProvidersStatus(){
		civilServiceDao.updateProvidersStatus();
	}
	...
	public List<CivilServiceVo> getThisModel(CivilServiceVo vo, Pager pager) {
		return civilServiceDao.getThisModel(vo, pager);
	}
