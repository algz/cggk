 ...
 * 
 */
public class ContractImportUploadServlet extends ExcelFileUploadServlet {
	...
	protected List processWorkbook(Workbook workbook, Map formField,HttpServletRequest req) throws FileUploadException {
		...
		try {
			...
			if(contractType.equals("1")){//合同导入
				procurementContractService.importContract(workbook, formField, noExistProducts, newMaterials);
			}else{//合同详情导入
