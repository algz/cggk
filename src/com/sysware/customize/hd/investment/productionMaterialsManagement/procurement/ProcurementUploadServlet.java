 ...
 * 
 */
public class ProcurementUploadServlet extends ExcelFileUploadServlet {
	...
	protected List processWorkbook(Workbook workbook, Map formField,HttpServletRequest req) throws FileUploadException {
		// 获取seam的上下文对象
		Lifecycle.beginCall();
		ProcurementDetailService procurementDetailService = (ProcurementDetailService) Component
				.getInstance("procurementDetail_ProcurementDetailServiceImpl",
						true);
		List<String> noExistProducts = new ArrayList<String>();
		List<Material> newMaterials = new ArrayList<Material>();
		try {
			procurementDetailService.importProcurementPlan(workbook, formField, noExistProducts, newMaterials);
